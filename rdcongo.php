	...
	<section class="mon_section">
			...
			<article class="mon_article">
				...
				<?php
					...
					$reponse = $bdd->query('SELECT * FROM news_simple WHERE categorie_nsimple = "Rdcongo" ORDER BY id_nsimple DESC LIMIT 0, 6');
		...
		<!--    	NOS NEWS AVEC LES PHOTOS ARRONDI-->
				...
				<?php
					...
					$reponse = $bdd->query('SELECT * FROM news_cool WHERE categorie_ncool = "Rdcongo" ORDER BY id_ncool');
					while ($donnees = $reponse->fetch())
					{
...
<?php 
					...
					$repon= $bdd->query('SELECT count(*) AS total FROM videos_courte WHERE categorie_video = "Rdcongo" ');
				...
				// La requête sql pour récupérer les messages de la page actuelle.	
					...
					$reponse = $bdd->query('SELECT * FROM videos_courte WHERE categorie_video = "Rdcongo" ORDER BY id_video DESC LIMIT '.$premiereEntree.', '.$videosParPage.'');
					while ($donnees = $reponse->fetch())
					{
						echo'
...
for($i=1; $i<=$nombreDePages; $i++) //On fait notre boucle
{
     ...
     {
          echo ' <a style="font-size:21px;" href="rdcongo.php?page='.$i.'">'.$i.'</a> ';
     }
}
echo '</p>';			
