...
<!--			
<div style="margin:inherit;display:inline-block;background-color:red;height:22px;position:fixed;left:800px;top:12px;">	
						<form method="GET" action="index.php">
						
							<label for="langue" style="color:yellow;font-weight:bold;">Select you language</label>	
							
								<select name="langue" placeholder="Language" required>
	...
	<section class="mon_section">
			...
			<article class="mon_article">
				...
				<?php
					$reponse = $bdd->query('SELECT * FROM news_simple_en ORDER BY id_nsimple_en DESC LIMIT 0, 6');
					
					while ($donnees = $reponse->fetch())
					{
				?>
					<h3 class="mon_h3"><?php echo' ' . $donnees['titre_nsimple_en'] . ' ' ?></h3>
					<h5 class="mon_h5"><?php echo' ' . $donnees['date_creation_ns_en'] . ' ' ?></h5>
					<p class="ma_paragraph">
						<?php echo' ' . $donnees['contenu_nsimple_en'] . ' ' ?>
					</p>
					<hr>
		...
		<article class="mon_article">
			...
			<div class="mon_divmini">
				<?php
					...
					$reponse = $bdd->query('SELECT * FROM news_cool_en ORDER BY id_ncool_en LIMIT 0, 4');
					while ($donnees = $reponse->fetch())
					{
						echo' <div class="mon_psmini"><a href="OMMmyAdmin/image/' . $donnees['nom_photo'] . '.' . $donnees['extension_photo'] . '"> <img src="OMMmyAdmin/image/' . $donnees['nom_photo'] . '.' . $donnees['extension_photo'] . '" alt="' . $donnees['description_ncool'] . '" width="150px"></a></div>'; ?>
							...
							<div class="mon_paralien" style="overflow:auto;border-bottom:0px solid blue;box-shadow:3px 3px 10px black inset;margin-bottom:5px;height:150px;">
								...
								<?php echo'<h3 class="mon_h5" style="text-align:center;background-color:purple;color:#ffffff;box-shadow:2px 3px 5px black;">' . $donnees['titre_ncool_en'] . '</h3> 	'; ?>
			...
			<!--    	NOS ACTUS EN CASECADE-->
				<?php
					...
					$reponse = $bdd->query('SELECT * FROM news_cool_en ORDER BY id_ncool_en LIMIT 4, 2');
					while ($donnees = $reponse->fetch())
					{
