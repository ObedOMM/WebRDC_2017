	...
	<ul class="nav">
		<li style="background:linear-gradient(to top,green,#003,#131,#589,#586);border-top-right-radius:5px;border-bottom-right-radius:5px;">
			<a href="" style="font-size:21px;background-color:transparent;">Language 
				<ul  class="dropdown">
					<li><a href="linafoot.php?langue=en">English</a></li>
					<li><a href="linafoot.php?langue=fr">French</a></li>
				</ul>
			</a>
		</li>
	</ul>		
			...
			<marquee Class="Scroller" behavior="scroll" direction="left" width="99.8%" height="30" scrolldelay="0" onmouseover="" onmouseout=""  style="background-color:white; border:0px solid black; margin:0px;box-shadow:5px 2px 7px black inset;font-family:arial, helvetica;font-size:25px;">
					...
					<strong><em>Welcome to www.yafoot.com!</em></strong>
						...
						<strong style="color:#333399;">
							Find here your football news of D.R.Congo, Africa and world, be informed in real time about football; sign in an account for accessing to all of website...
						</strong>
			</marquee>
	...
	<section class="mon_section">
			...
			<article class="mon_article">
				...
				<?php
					...
					$reponse = $bdd->query('SELECT * FROM news_simple_en WHERE categorie_nsimple_en = "Linafoot" ORDER BY id_nsimple_en DESC LIMIT 0, 6');
		...
		<!--    	NOS NEWS AVEC LES PHOTOS ARRONDI-->
				...
				<?php
					...
					$reponse = $bdd->query('SELECT * FROM news_cool_en WHERE categorie_ncool = "Linafoot" ORDER BY id_ncool_en');
					while ($donnees = $reponse->fetch())
					{
